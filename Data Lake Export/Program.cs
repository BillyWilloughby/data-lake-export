    ...
    namespace DLExport
    {
        class Program
        {
            ...
            private static void parseCommandLineParameters(string[] args)
            {
                ...
                if (_sql.Equals("") || _filename.Equals("") || _title.Equals("") || _connectionFile.Equals("") || _compassURL.Equals("")
                || args.Length == 0)
                {
                    Console.WriteLine("DataMover");
                    Console.WriteLine("©2006 - 2016 Billy Willoughby");
                    Console.WriteLine("Syntax:");
                    Console.WriteLine("\tSQL=\"select * from sometable\"");
                    Console.WriteLine("\tFilename=\"some output.xlsx\"");
                    Console.WriteLine("\tConnection=\"ION API Connection file\"");
            ...
            private static Table createNewTable(ref Document workingDoc, ref string[] rowHeaders, Font fontHeader, int fontSize)
            {
                ...
                foreach (string thisColumn in rowHeaders)
                {
                    //ws.Cells[rowIndex, intX + 1].Value = myReader.GetValue(intX);
                    thisHeader.Cells[intX].AddParagraph(rowHeaders[intX]);
                    thisHeader.Cells[intX].Shading = shadingHeader.Clone();
                    //if (pdfGfx.MeasureString(rowHeaders[intX], myHeaderFont).Width > columnWidth[intX].Width)
                    //{
                    //    columnWidth[intX] = pdfGfx.MeasureString(rowHeaders[intX], myStandardFont);
                    //    columnWidth[intX].Width = columnWidth[intX].Width + fontSize;
            ...
            private static int exportToPDF(DataTable queryResults)
            {
                try
                {
                    ...
                    pdfDDocument.Info.Subject = _title + " PDF Generated by SQLtoXLSX (PDF) ";
                    ...
                    {
                        rowHeaders[colIndex] = queryResults.Columns[colIndex].ColumnName;
                        if (_rowBreak != "" && _rowBreak.ToUpper() == queryResults.Columns[colIndex].ColumnName.ToUpper() && _rowBreakLocation == null)
                        {
                            _rowBreakLocation = colIndex;
                        }
                        colIndex = colIndex + 1;
                    }
                    ...
                    bool rowFlip = false;
                    int documentRowIndex = 1;  //Row one was created in create table function
                    ...
                    if (queryResults.Rows.Count > 0)
                    {
                        ...
                        while (queryResults.Rows.Count > documentRowIndex)
                        {
                            ...
                            {
                                Console.WriteLine("Reading Rows " + documentRowIndex + "+");
                            }

                            if (documentRowIndex == 2 && _rowBreak != "")
                            {
                            ...
                            foreach (string thisColumnNotUsed in rowHeaders)
                            {
                                thisRow.Cells[colIndex].AddParagraph(queryResults.Rows[tableRowIndex].ItemArray[colIndex].ToString());

                                if (pdfGfx.MeasureString(queryResults.Rows[tableRowIndex].ItemArray[colIndex].ToString(), myStandardFont).Width > columnWidth[colIndex].Width)
                                {
                                    columnWidth[colIndex] = pdfGfx.MeasureString(queryResults.Rows[tableRowIndex].ItemArray[colIndex].ToString(), myStandardFont);
                                    columnWidth[colIndex] = pdfGfx.MeasureString(queryResults.Rows[tableRowIndex].ItemArray[colIndex].ToString(), myStandardFont);
                                    columnWidth[colIndex].Width = columnWidth[colIndex].Width + fontSize;
                                }
                                colIndex = colIndex + 1;
                            }
                            documentRowIndex++;
            ...
            private static async Task<DataTable> runDataLakeAsync(string sqlFile)
            {
                ...
                string responseBody = await query.ReceiveString();

                dynamic dyn = JsonConvert.DeserializeObject(responseBody);

                string Status = dyn.status;
